function changeImage(imageSrc) {
    // Find the big image element by its ID
    const bigImage = document.getElementById("big-image");
    
    // Change the 'src' attribute of the big image to the one passed to the function
    bigImage.src = imageSrc;
}

function change(color) {
    document.getElementById("color").style.color = color;
    document.getElementById("color").textContent = color.charAt(0).toUpperCase() + color.slice(1);
}
